...
def realignedSequences(targetsite_sequence, chosen_alignment, errors=7):
	...
	for seq in targetsite_realignments:
		...
		for y_pattern, y_alignment in list(zip(match_pattern, match_realignments)):
...
"""
def analyze(sam_filename, reference_genome, outfile, annotations, windowsize, max_score, umi_len, idx_len):
...
def loadFileIntoArray(filename):
	with open(filename, 'rU') as f:
		...
		for line in f:
			...
			line_to_dict = dict(list(zip(keys, rest)))
...
def main():
	...
	parser.add_argument('--umi_len', help='UMI length',required=False,type=int,default=7)
	parser.add_argument('--idx_len', help='index length',required=False,type=int,default=8)

	args = parser.parse_args()

	annotations = {'Description': 'test description', 'Targetsite': 'dummy targetsite', 'Sequence': args.target}
	analyze(args.samfile[0], args.ref, args.outfile, annotations, args.window, args.max_score, args.umi_len, args.idx_len)
