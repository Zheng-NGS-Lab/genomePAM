...
def parseSitesFile(infile):
	...
	with open(infile, 'r') as f:
		...
		for line in f:
			...
			if len(line_items) == 45:
				target_seq_range = line_items[42]
				target_seq_type = line_items[43]
				target_seq_gene = line_items[44]
			...
			if no_bulge_offtarget_sequence != '' or bulge_offtarget_sequence != '':
				...
				if len(line_items) == 45:
					offtargets.append({'seq': no_bulge_offtarget_sequence.strip(),
								   ...
								   'range': target_seq_range.strip(),
								   'type': target_seq_type.strip(),
								   'gene': target_seq_gene.strip()
								   })
				else:
					offtargets.append({'seq': no_bulge_offtarget_sequence.strip(),
								   'bulged_seq': bulge_offtarget_sequence.strip(),
...
def visualizeOfftargets(infile, outfile, title, spacer, PAM):
	...
	for x,y in list(zip(tick_locations, tick_legend)):
	...
	PAM_index_ls = [PAM_index + off for off in range(0,len(PAM))]
	for i, c in enumerate(target_seq):
		y = y_offset
		x = x_offset + i * box_size
		if i not in PAM_index_ls:    # Previously: if i < PAM_index:
	...
	dwg.add(dwg.text('Range', insert=(x_offset + box_size * len(target_seq) + 72, y_offset + box_size - 3), style="font-size:15px; font-family:Courier"))
	dwg.add(dwg.text('Type', insert=(x_offset + box_size * len(target_seq) + 324, y_offset + box_size - 3), style="font-size:15px; font-family:Courier"))
	dwg.add(dwg.text('Gene', insert=(x_offset + box_size * len(target_seq) + 460, y_offset + box_size - 3), style="font-size:15px; font-family:Courier"))
	...
	for j, seq in enumerate(offtargets):
		...
		if no_bulge_offtarget_sequence != '':
			...
			for i, (c, r) in enumerate(list(zip(no_bulge_offtarget_sequence, target_seq))):
		...
		if bulge_offtarget_sequence != '':
			...
			line_number += 1
			y = y_offset + line_number * box_size
			for i, (c, r) in enumerate(list(zip(bulge_offtarget_sequence, realigned_target_seq))):
				x = x_offset + k * box_size
				if r == '-':
		...
		if no_bulge_offtarget_sequence == '' or bulge_offtarget_sequence == '':
			...
			dwg.add(reads_text)
			try:
				reads_text01 = dwg.text(seq['range'], insert=(box_size * (len(target_seq) + 1) + 76, y_offset + box_size * (line_number + 2) - 2),
									fill='black', style="font-size:15px; font-family:Courier")
				dwg.add(reads_text01)
				reads_text01 = dwg.text(seq['type'], insert=(box_size * (len(target_seq) + 1) + 328, y_offset + box_size * (line_number + 2) - 2),
									fill='black', style="font-size:15px; font-family:Courier")
				dwg.add(reads_text01)
				reads_text01 = dwg.text(seq['gene'], insert=(box_size * (len(target_seq) + 1) + 464, y_offset + box_size * (line_number + 2) - 2),
									fill='black', style="font-size:15px; font-family:Courier")
				dwg.add(reads_text01)
			except KeyError:
				print("No annotation...")
			...
			dwg.add(reads_text)
			try:
				reads_text01 = dwg.text(seq['range'], insert=(box_size * (len(target_seq) + 1) + 76, y_offset + box_size * (line_number + 1) + 5),
									fill='black', style="font-size:15px; font-family:Courier")
				dwg.add(reads_text01)
				reads_text01 = dwg.text(seq['type'], insert=(box_size * (len(target_seq) + 1) + 328, y_offset + box_size * (line_number + 1) + 5),
									fill='black', style="font-size:15px; font-family:Courier")
				dwg.add(reads_text01)
				reads_text01 = dwg.text(seq['gene'], insert=(box_size * (len(target_seq) + 1) + 464, y_offset + box_size * (line_number + 1) + 5),
									fill='black', style="font-size:15px; font-family:Courier")
				dwg.add(reads_text01)
			except KeyError:
				print("No annotation...")
