...
if [ -f identified/${sample}_identifiedOfftargets.txt ]; then
	...
	snpEff -Xmx8g -i bed $genome -o bed identified/${sample}.bed | awk -F"\t" '!/^#/{split($4,a,";");split(a[2],b,":");t=b[1];g=b[2];if(b[1]=="Intron"){g=b[6]}else if (b[1]=="Upstream"||b[1]=="Downstream"){g=b[4]}else if (b[1]=="Exon"){t="Exon";g=b[6]}else if (b[1]=="Gene"){t="Exon"};if($1!~/^chr/){$1=sprintf("chr%s",$1)}printf "%s\t%s:%d-%d\t%s\t%s\n",a[1],$1,$2+1,$3,t,g}' > identified/${sample}.anno
