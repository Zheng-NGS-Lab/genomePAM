        ...
        try:
            ...
            for sample in self.samples:
                ...
                identifyOfftargetSites.analyze(samfile, self.reference_genome, self.identified[sample], annotations,
                                               self.window_size, self.max_score, self.umi_len, self.idx_len)
        ...
        for sample in self.samples: ## 3/6/2020 Yichao solved: visualization stopped when one sample failed
            if sample != 'control':
                ...
                try:
                    ...
                    try:
                        self.PAM
                        visualizeOfftargets(infile, outfile, title=title, spacer=self.target_seq, PAM=self.PAM)
                    except:
                        visualizeOfftargets(infile, outfile, title=title, spacer=self.target_seq, PAM="NGG")
                except Exception as e:
...
def parse_args():
    ...
    identify_parser.add_argument('--umi_len', required=False, type=int, default=7)
    identify_parser.add_argument('--idx_len', required=False, type=int, default=8)
    ...
    visualize_parser.add_argument('--PAM', required=False, default='NGG')
    visualize_parser.add_argument('--target_seq', required=False)
...
def main():
    ...
    if args.command == 'all':
        ...
        if args.umi_len is not None:
            umi_len = args.umi_len
        else:
            umi_len = 7

        if args.idx_len is not None:
        ...
        if args.target_seq is not None:
            target_seq = args.target_seq
        else:
            target_seq = "AAAGGCTGCTGATGACACCT" + PAM # Intellia TTR sgRNA + PAM supplied

        g = GuideSeq()
        g.output_folder = os.path.dirname(args.outfolder)
        g.PAM = PAM
        g.target_seq = target_seq
